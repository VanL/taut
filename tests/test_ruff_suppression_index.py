...
def contain_failure(callback):
    try:
        callback()
    except Exception:  # noqa: BLE001 approved [DOM-10.2.1] [RUFF-SUP-001] exception
        return None
""",
        encoding="utf-8",
    )
...
def test_marker_like_string_literal_is_inert(tmp_path: Path) -> None:
    ...
    source.write_text(
        ...
        + '\nMARKER_EXAMPLE = "# noqa: BLE001 approved [DOM-10.2.1] '
...
class Context:
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:  # noqa: PYI036 approved [DOM-10.2.1] [RUFF-SUP-001] exception
        return None
""",
        encoding="utf-8",
    )
...
def test_registry_terms_in_an_ordinary_prose_comment_are_inert(
    ...
    source.write_text(
        source.read_text(encoding="utf-8")
        + "\n# Documentation discusses approved [DOM-10.2.1] and RUFF-SUP-001.\n",
        encoding="utf-8",
    )
