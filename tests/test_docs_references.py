...
2. Spec-code claims — a bracketed ``[TAUT-N]``/``[TAUT-N.M]``,
   ``[IAN-N]``/``[IAN-N.M]``, or ``[SUM-N]``/``[SUM-N.M]`` code, which must
   resolve to a heading in ``docs/specs/02-taut-core.md``,
   ``docs/specs/03-identity-addressing-notifications.md``, or
...
- Path-claim sources: ``docs/implementation/*.md``, ``docs/specs/*.md``,
  ...
  ``tests/**/*.py``, ``extensions/**/*.py``, ``docs/implementation/*.md``,
  ``docs/specs/*.md``.
...
SPEC_CODE_RE = re.compile(r"\[(?:TAUT|IAN|SUM)-\d+(?:\.\d+)?\]")
...
def _valid_spec_codes() -> frozenset[str]:
    ...
    for spec in SPEC_FILES:
        # Every registered spec file must exist: SUM was registered when
        # docs/specs/04-summon.md was promoted, so a missing file is
        # governance drift, not a soft skip.
