...
| 2026-08-17, hosted search-result synchronization | The 0.9.2 pre-tag Ubuntu Python 3.14 job `95560962858` in run `32086814821` completed the exact intent-tokened search snapshot, but the handler test asserted the logical anchor before `_render_messages()` ran its deferred `_restore_transcript_anchor()` callback. The other four TUI jobs passed, including Windows. The test now delegates and observes the selected hit's exact restore, signals after its following refresh, and awaits both owned events before retaining every snapshot, row, target, and anchor assertion. | Classified as a test-observation defect unless the exact restore completes with wrong state. Time remains only a missing-callback cap; no production behavior, matrix, timeout, or assertion changed. |
