...
| 0.5.4 release resubmission | GitHub Actions coverage and Windows matrices | Coverage evidence rendered host-native separators, so every Windows lane rejected a valid missing-evidence record. The coverage lane also exposed full reactor cleanup running inside SIGINT signal context, where runner load and instrumentation could strand shutdown past the test deadline. | Accepted red-first. Evidence labels now use POSIX paths. `BaseReactor` SIGINT publishes stop/wake state and raises; normal unwind remains the sole cleanup owner. Focused path, ordinary-SIGINT, and replacement-reentrant-SIGINT tests are required before retagging. |
| 0.5.4 correction review | Independent lifecycle reviewer | The first SIGINT correction relied on `run_until_stopped()` and the CLI for cleanup. A signal during running-state publication, handler installation, or drive-owner claim could bypass reactor cleanup boundaries and leave the installed handler or owned resources live. | Accepted red-first. `run_forever()` now protects running-state publication, pre-records and restores the prior handler, and performs idempotent stop from its outer `finally`. Deterministic publication, install-interruption, and pre-claim interruption tests assert restoration, cleanup, and cleared running state. |
| 0.5.4 retag precheck | Local serialized PTY lane | The attach/inject serialization test joined the bridge immediately after detach without consuming the bridge's terminal-reset output. The product had recognized detach and was correctly waiting in `tcsetattr(TCSADRAIN)`; the synthetic PTY peer intermittently made that restore look like a detach failure. | Accepted as a test-ownership correction, not a timeout increase. The peer now drains and asserts the reset blast before joining. The exact failing test passed 50/50 focused runs. |
