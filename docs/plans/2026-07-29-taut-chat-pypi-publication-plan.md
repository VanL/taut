...
| R8 | Independent I13 review passed the command ordering and fail-closed dependency boundary. It identified the nested uv call behind `bin/pytest-pg`, the prepared-environment prerequisite, and wording that could overstate MCP's runtime pin proof. | Accepted nested protection with a Taut-owned `TAUT_PG_UV_NO_SYNC=1` release override; uv does not propagate its own `UV_NO_SYNC` setting to the child command. Retained standalone `bin/pytest-pg` syncing outside the release boundary. Accepted the prepared dev environment as an explicit release prerequisite: missing tools fail rather than changing the graph during release. Clarified that MCP's proof is its retained static lock, while the canonical root interpreter supplies the runtime Ruff proof. | Claude Opus, 2026-08-05, read-only PASS; the first real run showed the nested command still installing under `UV_NO_SYNC=1`, so it was stopped before remote action. The corrected real PG run echoed `--no-sync` on all three nested uv commands, passed 242 shared and 14 extension tests, and left root `uv.lock` absent. |
| R9 | Final I13 follow-up checked the standalone/release split and the real nested proof after the first follow-up review blocked on asymmetric tests. | Passed. Standalone explicitly rejects `--no-sync` while retaining editable-extension and argument-placement assertions; release mode explicitly requires it. No release blocker remains. | Claude Opus, 2026-08-05, read-only PASS; the 242+14 real PG run closes the end-to-end finding. |
| I14 | Fresh canonical run `31038957793` passed the corrected repository policy tests on Windows 3.12 and 3.13, then failed only on Windows 3.14 because the isolated BaseReactor SIGINT child produced no stdout before the parent's three-second timeout. The production probe had passed locally and on the other Windows versions; the empty protocol distinguished pre-result delay but could not distinguish child startup from behavior. | Accepted as a test-harness scheduling race, not evidence of a production watcher race. The child now emits structured readiness after imports. The parent applies a distinct bounded startup watchdog, starts the unchanged three-second behavior watchdog only after readiness, and reports the two failures separately. The real probe joins its watchdog and sentinel in one xdist group. One reader thread owns each pipe for the child's full lifetime; the parent never mixes buffered reads with `communicate()`. | The seven focused cases passed, including both watchdog branches and every protocol-error branch; 20/20 grouped xdist stress runs passed; the complete 80-test watcher file passed under `-n auto --dist loadgroup`; Ruff and mypy passed. The failed hosted run and all four failed publication gates remain retained evidence, with no GitHub Release or PyPI publication. |
| R10 | The first I14 review blocked release because the initial handshake mixed a `TextIOWrapper.readline()` thread with `Popen.communicate()`, which could lose buffered result bytes, and cleanup could run two readers against stdout. | Accepted. A single stdout-draining thread and a single stderr-draining thread now own the pipes until EOF; the parent uses only `wait`, then joins the readers. Early EOF has a distinct failure contract. | Claude Opus, 2026-08-05, read-only BLOCKED; all blocking findings were corrected before the follow-up. |
| R11 | I14 follow-up reviewed the corrected single-reader lifecycle, early EOF, strict post-readiness watchdog, and xdist grouping. It passed release, while noting missing firing tests for invalid and unexpected startup status and an unwrapped post-kill reap timeout. | Accepted both non-blocking findings rather than carrying debt: explicit child modes now fire both protocol-error branches, and post-kill reap timeout becomes a structured assertion. Confirmed the hung-child test retains the same xdist marker. | Claude Opus, 2026-08-05, read-only PASS; final focused, stress, full-file, lint, and type evidence is recorded before release. |
| R12 | Final I14 review rechecked the corrections requested by R11, the whole pipe lifecycle, timeout semantics, every related xdist marker, test determinism, and documentation truth. | Passed without a release blocker or unresolved definition-of-done gap. The final code has no `communicate()` call in the probe path, each pipe has one lifetime reader, protocol branches have firing tests, and the production behavior clock remains three seconds after readiness. | Claude Opus, 2026-08-05, read-only PASS after seven focused cases, 20/20 final grouped stress runs, the complete 80-test watcher file, Ruff, mypy, and `git diff --check` all passed. |
...
| I17 | The post-release evidence commit's MCP workflow failed once on Python 3.11 because a pacing integration test slept for 0.2 seconds and asserted that the second update had not arrived, although its timestamps showed the runner resumed 0.859 seconds later and production had respected the 0.5-second boundary. | Remove the scheduler-dependent intermediate assertion. Prove the negative and exact-boundary behavior directly on the child reactor with a controlled monotonic clock, while retaining the real-thread test for immediate first wake, eventual second wake, content, timestamp gap, and resource ownership. Production code remains unchanged. | A deliberate 0.7-second master-loop stall reproduced the exact failure with correctly paced timestamps. The fake-clock and integration tests passed, the complete resource file passed, the full non-PG MCP suite passed with 207 tests, and the integration test passed 25 fresh-process repetitions. |
| R16 | Independent I17 review checked whether the direct private-reactor seam weakened the end-to-end pacing proof. | Passed. The fake-clock test is the correct owner-level proof and restores both the pre-deadline negative assertion and exact-deadline upper bound that the integration test cannot reliably observe. Accepted non-blocking hardening to derive times from the production interval, prove pre-deadline state is unchanged, prove no emission without a pending wake, and document deliberate backstop isolation. | Claude Opus, 2026-08-05, read-only PASS after focused, stress, full-suite, Ruff, and mypy evidence. |
