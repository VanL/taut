...
| 2026-08-11 | Independent final release-evidence audit | No blocker. The reviewer independently verified all tag SHAs; exact producer and gate identities; public immutable two-asset releases; exact non-yanked PyPI filenames and full digests; per-file Sigstore statements, certificate identities, commit, runner, and `pypi` environment; hosted timings; and documentation gates. | Completion evidence accepted; commit this final record before closure. |
...
| 2026-08-11 | Successful 0.8.5 recovery | The normal coordinated helper ran without `--skip-checks` at `f4830b0`: 1,836 root tests, installed-wheel tests, shared and PG-only PostgreSQL tests, both Summon lanes, eight strict live-harness tests, the real local-LLM smoke, 252 MCP tests, Ruff, mypy, four empty-dist builds, and the compatibility wheel matrix all passed. Exact-SHA root `31529035350`, PG `31529035344`, and MCP `31529035392` passed before leased tag replacement. Publication gates `31529801614`, `31529795139`, `31529796864`, and `31529799383` then passed without reruns. | All four tags resolve to `f4830b0`. All four GitHub Releases are public, non-prerelease, immutable, and contain exactly one wheel and one sdist. PyPI exposes the same eight filenames and SHA-256 digests, all non-yanked; its integrity endpoint binds every file to one Sigstore bundle from `VanL/taut`, environment `pypi`, and the correct top-level gate. A fresh Python 3.14 environment installed and imported all four packages at 0.8.5. |
| 2026-08-11 | Hosted speed evidence | The exact successful Windows cells completed in 6m02s (3.11), 4m20s (3.12), 4m47s (3.13), and 3m52s (3.14); the whole root workflow completed in 8m30s. Prior green full-suite Windows cells took 14 to 18.5 minutes, with one failure reaching the 20-minute cap at 35%. | Exact factor coverage reduced Windows cell wall time by roughly 57% to 79% while the executable oracle proved exact union, disjointness, and xdist-group integrity. Coverage remained in the Ubuntu 3.13 producer rather than a separate workflow. |
