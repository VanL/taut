...
|------|--------|----------|------|
| `2026-09-24-windows-tui-determinism-root-cause-plan.md` | draft | no | Class 4: root-cause deep dive of the 2026-09-23 Windows TUI streak and S4, a written determinism model, event-based waits with split budgets, one elicitation test per root-cause class, and a soak-based sufficiency criterion; inherits S4. Awaits plan review. |
| `2026-09-24-tui-text-selection-and-clipboard-plan.md` | draft | no | Class 5 hardened: framework text selection in the transcript (today blocked by the option list), `y` and 500 ms auto-copy through OSC 52, copied text pinned to the policy-filtered display form. Awaits plan review. |
| `2026-09-24-cli-read-delivery-and-agent-recipes-plan.md` | draft | no | Class 5 hardened: CLI `read` advances cursors only through delivered records, `read -q` is a usage error and `list -q` becomes the documented poll, repeated `join` is silent, the short-form reply suffix is removed in favor of the one 19-digit id, unusable-storage diagnostics tell the truth. Awaits the `inbox -q` confirmation and plan review. |
| `2026-09-24-identity-claim-boundary-plan.md` | draft | no | Class 5 hardened: narrows [IAN-3.3] step-4 anchor matching so a parent agent cannot capture a child agent, specifies host-id derivation and its explicit fallback, routes read verbs through read-only resolution. Awaits owner rule choice and plan review. |
| `2026-09-24-watch-interrupt-drain-plan.md` | draft | no | Class 5 hardened: replaces the reactor's in-frame `KeyboardInterrupt` with flag-and-drain, declares the interrupted-watch exit code, corrects the [TAUT-8.5] tuning-knob name. Awaits owner exit-code choice and plan review. |
| `2026-09-24-mcp-not-found-result-contract-plan.md` | draft | no | Class 5 hardened: mutating MCP tools report not-found as `isError` per a fixed empty-versus-error table, adds the two missing [MCP-12]/[MCP-10] gates, removes the research-preview Claude channel adapter. Awaits owner confirmation of the removal and plan review. |
| `2026-09-24-summon-controlling-terminal-and-live-lane-plan.md` | draft | no | Class 5 hardened: POSIX provider gets a controlling terminal through the shared spawn owner, [SUM-11] states the driver-crash guarantee, fixtures reap process groups on every path, live-harness lane keeps its default and loses its false skips, and a host-PTY scenario lane drives the real CLI through attach, detach chord, reattach, and dismiss. Awaits plan review. |
| `2026-09-24-tui-participation-loop-plan.md` | draft | no | Class 5 hardened: sticky tail pinning under a named viewport owner, times instead of raw ids in transcript rows, real rapid-resize burst test replacing dead code, truthful TUI launch exit code, labeled DM search hits. Awaits plan review. |
| `2026-09-24-release-gate-corrections-plan.md` | draft | no | Class 4: PG lockfile test asserts git tracking not disk presence, PyPI post-upload budget widened to about five minutes, `--skip-checks` help names the dropped live lane, dry-run warns on a missing CHANGELOG heading. Awaits plan review. |
| `2026-09-24-review-inputs-and-doc-gates-plan.md` | draft | no | Class 3+P: review briefs carry the [THEORY-5] rejected alternatives in force, citation gate accepts nested codes, retired-ledger SHAs are resolved, DOM-15 fixture negatives are structural, xdist lessons marked superseded, plan-tier convergence decided. Awaits owner ratification and review. |
