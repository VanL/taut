...
|------|--------|----------|------|
| `2026-09-03-summon-unified-pty-cross-platform-plan.md` | active | no | Class 5 reviewed plan to remove the Claude structured adapter and terminal-mode/session protocol, add one cross-platform PTY/ConPTY path, and run all non-platform-specific Summon tests on every OS; fresh-eyes and Claude cross-model reviews passed. |
