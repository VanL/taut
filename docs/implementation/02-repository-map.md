...
| `bin/check-core-summon-wheel-matrix.py` | Isolated installed-artifact checker for the `taut-chat` core/current-Summon pair, exact distribution metadata, live control behavior, incompatible current-core floors, the historical `Requires-Dist: taut` rename diagnostic, and the admitted historical MCP/current-core stdio lifecycle canary |
| `bin/build-and-check-release-wheels.py` | Fresh-build owner, or paired explicit-current-wheel consumer in canonical CI, that supplies current artifacts and invokes the checker that builds both immutable historical release-source wheels |
