...
| `[RUFF-SUP-035]` | `C901` | `1` directive; raw: `C901=1` | One Windows Job Object finalizer owns graceful observation, zero-active retirement, leader reap, job-handle release, kill-on-close fallback, and stable terminal-error replay | deterministic and real Windows finalization, timeout, zero-active, reap, and idempotence tests | Split zero-active, reap, and job-handle cleanup owners, or helpers that duplicate finalization state | Process-domain plan and implementation independently reviewed; user-authorized implementation 2026-08-25; moved owner reconciled under the independently judged Ruff correction 2026-08-28. |
...
| `[RUFF-SUP-091]` | `BLE001` | `1` directive; raw: `BLE001=1` | One failed Windows spawn transaction contains arbitrary retire, stream, native-handle, and fallback-reap cleanup failures while preserving the original setup or control-flow primary and ordered cleanup notes | per-boundary control-flow cleanup characterization plus deterministic Win32 setup-failure, handle-release, fallback-reap, and error-translation tests | Four repeated broad catches; narrowing to `Exception`; or a context manager that hides the same broad boundary behind `__exit__` | All five Windows spawn candidates independently judged net positive; user-requested correction 2026-08-28. |
Global raw-`noqa` inventory: `BLE001=133`, `C901=38`, `DTZ006=1`, `F401=1`, `FLY002=16`, `FURB122=1`, `LOG001=1`, `N999=6`, `PYI041=1`, `RUF015=2`, `S110=3`, `SIM115=3`, `SIM117=7`, `TRY004=12`
...
| `[RUFF-SUP-035]` | `extensions/taut_summon/taut_summon/_win32_job.py::WindowsJobDomain.finalize` | 1 | `C901=1` |
...
| `[RUFF-SUP-091]` | `extensions/taut_summon/taut_summon/_win32_job.py::_WindowsSpawnAttempt._attempt_cleanup` | 1 | `BLE001=1` |
