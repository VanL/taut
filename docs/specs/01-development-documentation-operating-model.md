...
The vendored watcher preserves exact upstream suppression comments. The scanner
maps only the three exact Weft TS-3.1 annotations in `taut/watcher.py` to the
local groups 093–095 below. All existing rule, cardinality and raw-finding
checks still apply; foreign markers elsewhere or altered markers fail closed.
...
match `RUFF-SUP-[0-9]{3}`, and are never reused after retirement. The checker
validates the current registry and its source references. Reviewers check
retirement history in Git when assigning a group ID; tests do not maintain a
second retirement ledger. A temporary group also names the active plan task
...
| `[RUFF-SUP-066]` | `BLE001` | `9` directives; raw: `BLE001=9` | MCP process and workspace reactors contain arbitrary child, callback, transport, plugin, and shutdown failures at the owning reactor boundary | MCP routing, crash, cancellation, notification, and shutdown suites | Narrowing third-party or user callbacks, or propagating across event-loop and thread ownership | Independent review passed; user-approved exact fields 2026-08-05. |
| `[RUFF-SUP-067]` | `BLE001` | `6` directives; raw: `BLE001=6` | Summon control, driver, and PTY lifecycle owners preserve primary-error precedence and contain arbitrary child, adapter, cleanup, and give-up/offer diagnostic failures | Summon teardown, signal, control, PTY, restoration, and give-up diagnostic-containment suites plus the setup-recovery offer-excerpt proof | Guessed exception lists or split cleanup and diagnostic owners | Independent review passed; user-approved exact fields 2026-08-05; give-up diagnostic boundary independently reviewed and owner-authorized 2026-08-19; offer-excerpt capture added under the independently reviewed 2026-08-19 TUI setup-recovery offer plan. |
...
| `[RUFF-SUP-070]` | `BLE001` | `29` directives; raw: `BLE001=29` | Concurrency and real-process test harnesses capture any `BaseException` from worker threads so the owning test can join and assert the exact outcome | Affected tests plus their live-thread, join, and captured-failure assertions | `Exception`-only capture, hidden thread warnings, or mocked concurrency | Independent review passed; user-approved exact fields 2026-08-05; foreground-readiness cases added under the independently reviewed TUI plan 2026-08-13; terminal-attach case added under the independently reviewed Summon plan 2026-08-17; gate-wiring case added under the independently reviewed 2026-08-19 TUI setup-recovery offer plan; cross-platform terminal-probe relay retained after independent review 2026-09-14. |
| `[RUFF-SUP-071]` | `BLE001` | `16` directives; raw: `BLE001=16` | Race, transient-service, and harness tests retain any contender or fixture `Exception` as diagnostic or expected-race evidence | Affected PostgreSQL, Summon, and core race and diagnostic tests | Guessed production exception lists in adversarial harnesses or discarding evidence without a later assertion or diagnostic | Independent review passed; user-approved exact fields 2026-08-05; three real blocked-injection probes added by independently reviewed 0.9.0 remediation 2026-08-14; Summon driver fixture teardown now aggregates every ordinary cleanup exception without abandoning later owned processes. |
...
| `[RUFF-SUP-085]` | `BLE001` | `20` directives; raw: `BLE001=20` | TUI domain, extension, and syntax-provider boundaries render arbitrary backend, plugin, and provider failures inline without losing the active UI | TUI domain-action, extension-startup, command-syntax-provider, search, and Summon error-path tests | Guessing third-party/provider exception classes or allowing a domain or provider failure to terminate the Textual event loop | TUI command-mirror, Summon attach-handoff, and deep-review remediation implementations independently reviewed; owner-authorized implementation 2026-08-17..18. The 2026-09-14 rename result boundary retained locality after an independent suppression-free refactor trial proved less maintainable. The 2026-09-15 draft-recovery and rename-validation boundaries retain arbitrary domain failures for inline reporting. |
| `[RUFF-SUP-086]` | `BLE001` | `11` directives; raw: `BLE001=11` | Auxiliary TUI scheduling, logging, readiness, and presentation callbacks remain best-effort and cannot replace the owning domain or driver outcome | TUI callback, log-bridge, readiness, search-result, and presentation-containment tests | Letting auxiliary presentation failure escape, or logging recursively through a failed presentation path | TUI plans and implementations independently reviewed; user-authorized release preparation and deep-review remediation 2026-08-13..18. |
...
| `[RUFF-SUP-090]` | `BLE001` | `7` directives; raw: `BLE001=7` | Debug evidence construction, sink work, and retained-Textual-failure inspection remain subordinate to an already-owned primary outcome, including when arbitrary local representation, backend cleanup, subprocess teardown, or compatibility inspection raises a control-flow exception | Debug event, hostile-local, queue-fault, action-timeout, recursion, original-outcome, and real Textual fatal-callback tests | Narrow exception guesses that let auxiliary capture replace the primary failure, or a second generic crash framework | Debug failure-capture plan independently reviewed; user-authorized implementation 2026-08-14. |
| `[RUFF-SUP-092]` | `BLE001` | `1` directive; raw: `BLE001=1` | BrokerSession cleanup retains arbitrary backend failures without losing the primary application failure, while one reactor finalizer keeps strategy, direct queues, runtime, recycle, scope-close, and retry ownership together | client/reactor constructor rollback, active-operation close refusal and retry, cleanup-failure isolation, and real retained-peer tests | Guessing backend exception classes, clearing ownership before successful close, or splitting the finalizer into competing cleanup owners | BrokerSession integration plan independently reviewed; user-authorized implementation 2026-09-15. |
| `[RUFF-SUP-093]` | `C901` | `1` directive; raw: `C901=1` | Copied Weft topology transaction keeps candidate build, publication, rollback and retirement atomic | Owner/foreign topology, stop race and source-body equivalence tests | Splitting copied transaction creates a maintained fork | User-authorized reactor restoration; exact vendoring reviewed. |
| `[RUFF-SUP-094]` | `SIM102` | `1` directive; raw: `SIM102=1` | Copied candidate waiter cleanup preserves upstream transaction structure | Topology rollback and source-body equivalence tests | Cosmetic divergence in copied scheduler | User-authorized reactor restoration; exact vendoring reviewed. |
| `[RUFF-SUP-095]` | `BLE001` | `2` directives; raw: `BLE001=2` | Copied topology failure boundary restores state for arbitrary backend and signal failures | Topology rollback, SIGINT and source-body equivalence tests | Guessing backend exception types or leaving foreign request unresolved | User-authorized reactor restoration; exact vendoring reviewed. |

Global raw-`noqa` inventory: `BLE001=132`, `C901=35`, `DTZ006=1`, `F401=1`, `FLY002=16`, `FURB122=1`, `LOG001=1`, `N999=6`, `PYI041=1`, `RUF015=2`, `SIM102=1`, `SIM115=3`, `SIM117=7`, `TRY004=11`
...
| `[RUFF-SUP-030]` | `extensions/taut_summon/taut_summon/_pty_posix.py::PosixPtyHandle.attach` | 1 | `C901=1` |
| `[RUFF-SUP-031]` | `extensions/taut_summon/taut_summon/_pty_posix.py::PosixPtyHandle._event_stream` | 1 | `C901=1` |
...
| `[RUFF-SUP-066]` | `extensions/taut_mcp/taut_mcp/_process_reactor.py::ProcessReactor._finish_claude_attempt`; `extensions/taut_mcp/taut_mcp/_process_reactor.py::ProcessReactor._signal_claude_change`; `extensions/taut_mcp/taut_mcp/_process_reactor.py::ProcessReactor._signal_modern_change`; `extensions/taut_mcp/taut_mcp/_process_reactor.py::ProcessReactor.ensure_workspace`; `extensions/taut_mcp/taut_mcp/_workspace_reactor.py::_WorkspaceReactor._execute_command`; `extensions/taut_mcp/taut_mcp/_workspace_reactor.py::_WorkspaceReactor._publish_snapshot_if_due`; `extensions/taut_mcp/taut_mcp/_workspace_reactor.py::_WorkspaceReactor._refresh_after_command`; `extensions/taut_mcp/taut_mcp/_workspace_reactor.py::_validate_candidate`; `extensions/taut_mcp/taut_mcp/_workspace_reactor.py::run_workspace_reactor` | 9 | `BLE001=9` |
| `[RUFF-SUP-067]` | `extensions/taut_summon/taut_summon/_driver.py::SummonDriver._release`; `extensions/taut_summon/taut_summon/_driver.py::SummonDriver._run_terminal_attach`; `extensions/taut_summon/taut_summon/_driver.py::SummonDriver._teardown_generation`; `extensions/taut_summon/taut_summon/_driver.py::_agent_capture`; `extensions/taut_summon/taut_summon/_pty_posix.py::PosixPtyHandle.close` | 6 | `BLE001=6` |
...
| `[RUFF-SUP-085]` | `extensions/taut_tui/taut_tui/app.py::TautApp._apply_action_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_channel_rename_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_deletion_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_draft_recovery`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_form_action_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_navigation_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_optional_conversation`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_owned_exit_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_search_context`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_send_result`; `extensions/taut_tui/taut_tui/app.py::TautApp._command_syntax`; `extensions/taut_tui/taut_tui/app.py::TautApp._complete_summon_start`; `extensions/taut_tui/taut_tui/app.py::TautApp._dispatch_summon_command`; `extensions/taut_tui/taut_tui/app.py::TautApp._dispatch_system_or_summon_action`; `extensions/taut_tui/taut_tui/app.py::TautApp._submit_channel_rename`; `extensions/taut_tui/taut_tui/app.py::TautApp._submit_dump`; `extensions/taut_tui/taut_tui/app.py::TautApp.on_mount`; `extensions/taut_tui/taut_tui/app.py::TautApp.on_terminal_attach_confirmation_request`; `extensions/taut_tui/taut_tui/screens.py::SearchScreen._apply_results`; `taut/commands/syntax.py::discover_command_syntax` | 20 | `BLE001=20` |
| `[RUFF-SUP-086]` | `extensions/taut_tui/taut_tui/app.py::TautApp._accept_summon_log_from_worker`; `extensions/taut_tui/taut_tui/app.py::TautApp._accept_summon_ready_from_worker`; `extensions/taut_tui/taut_tui/app.py::TautApp._apply_summon_return`; `extensions/taut_tui/taut_tui/app.py::TautApp._watch_future`; `extensions/taut_tui/taut_tui/app.py::_dismiss_decided_confirmation`; `extensions/taut_tui/taut_tui/screens.py::SearchScreen.on_input_submitted`; `extensions/taut_tui/taut_tui/summon.py::SummonLogBridge._deliver`; `extensions/taut_tui/taut_tui/summon.py::TerminalAttachConfirmationRequest._notify`; `extensions/taut_tui/taut_tui/summon.py::_ForwardingHandler.emit` | 11 | `BLE001=11` |
...
| `[RUFF-SUP-088]` | `extensions/taut_tui/taut_tui/summon.py::TuiSummonInteraction._terminal_lease` | 1 | `C901=1` |
...
| `[RUFF-SUP-090]` | `extensions/taut_tui/taut_tui/_launch.py::_retained_textual_fatal`; `taut/debug.py::_formatted_traceback`; `taut/debug.py::_safe_cwd`; `taut/debug.py::_safe_repr`; `taut/debug.py::_safe_text`; `taut/debug.py::_write_local`; `taut/debug.py::capture_exception` | 7 | `BLE001=7` |
| `[RUFF-SUP-092]` | `taut/_cleanup.py::capture_cleanup_failure` | 1 | `BLE001=1` |
| `[RUFF-SUP-093]` | `taut/watcher.py::MultiQueueWatcher._apply_topology_mutation_on_owner` | 1 | `C901=1` |
| `[RUFF-SUP-094]` | `taut/watcher.py::MultiQueueWatcher._apply_topology_mutation_on_owner` | 1 | `SIM102=1` |
| `[RUFF-SUP-095]` | `taut/watcher.py::MultiQueueWatcher._run_owner_topology_transaction` | 2 | `BLE001=2` |
