...
- The driver consumes control queues with a long-lived control reactor on a
  ...
  `simplebroker>=7.1.0` required for the supported reactor lane. Version
  ...
  broadcast; `simplebroker>=7.1.0` is the repository-wide supported floor,
  aligned with
...
- Firing tests cover invalid partial record evidence, indeterminate takeover,
  ...
  the 7.1.0 floor, and ordered release invocation with fresh built artifacts.
