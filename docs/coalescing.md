...
- **Plans.** The fold unit is the individual plan; progress is tracked by
  ...
  count. **A plan-tier sweep audits every eligible candidate** and
  soft-retires each one that passes the four-part harvest gate; a
  candidate is deferred only with a per-plan reason in the run log.
  Oldest-first is the lessons tier's cursor discipline and does not apply
  here: retiring a fixed batch of the oldest candidates per sweep cannot
  converge against plan inflow (owner direction 2026-09-24, after the
  2026-08-08..2026-09-23 sweeps each retired four while inflow ran near
  nine a week; SimpleBroker's sweeps, which harvest all eligible
  candidates with parallel independent audits, keep its tier at or under
  threshold). Parallel independent audits are the expected shape for a
  large backlog.
...
|------|-------------------|------------|
| Lessons | 2026-08-18 | `70ac379e2` |
| Plans | (progress is the Retired Plans ledger, not a cursor — four four-plan soft-retirement batches recorded through 2026-09-23; all-eligible sweeps from 2026-09-24) | per-plan, see ledger |
...
|------|------------------------------|-----------------|-----------------|-----------------|
| Lessons | 2026-09-24 (additive), 2026-09-23 fold source `70ac379e2` | 96 dated entries after the derivation repair: 34 cold entries dated 2026-07-27..2026-08-18 were examined; 2 folded; 32 kept verbatim; watermark advanced to 2026-08-18; 14 entries dated 2026-08-25..2026-09-22 remain ahead of the watermark and inside the 30-day age floor; 0 cold eligible against threshold 20 | Every cold entry was dispositioned. The two folded entries were already owned. The 32 kept entries are named candidates in the 2026-09-23 lessons run-log line, not unexamined material behind the cursor | Recount when cold eligible entries after 2026-08-18 reach 20. The oldest kept-ahead entry, 2026-08-25, reaches the age floor on 2026-09-24 and stays under threshold until more entries land. Distill the named kept clusters only in an owner-authorized Class 3+P unit, or when their active plans close and a later sweep is asked to distill those clusters |
| Plans | 2026-09-24, all-eligible sweep over 100 candidates at `c894059` | 127 plan files: 100 retired-pending, 7 completed (4 deferred with named blockers plus 3 completed since the audit inventory), 9 active, 10 draft, 1 status-review; harvest candidates = 4 | Deferred with per-plan reasons: reactor-restoration (S0 owner maps cited as governing by two implementation docs), coordinated-0-9-8 (`uv --isolated` precheck rule needs [TAUT-12.5] text), summon-stop-release-race (same-process driver-evidence limitation needs [SUM-8] text), test-quality-remediation (oracle rules need testing-patterns runbook text). Each requires spec or runbook text, which is owner-gated. | Recount when any deferred plan's spec/runbook text lands, or when the completed count exceeds 8 again |
| Promotion | 2026-09-23, source `70ac379e2` | 3 candidate themes, each with new August citations: xdist/scheduler containment; Summon readiness and input handoff; causal time-oracles. No new unowned skill theme | The August citations extend the same three workflows. Release, TUI, Windows lifecycle, extension ownership, and dependency-floor clusters stay product-owned or inside active plans. Amending testing-patterns Pattern 8, or any other durable guidance, remains owner-gated | A Class 3+P runbook or implementation-note amendment is authorized, a new unowned workflow reaches 3 independent citations, or the active Windows, TUI, Summon, publication, extension, and compatibility plans close and later independent citations establish recurrence |
...
|------|---------|------------|-------|
| 2026-09-24 | Plans (soft retirement, all-eligible) | tree at `c894059`; per-plan source SHAs in the Retired Plans ledger | **Soft-retired 96 of 100 eligible plans after eight parallel independent four-gate audits** (owner-authorized catch-up sweep under the all-eligible rule promoted the same day). 82 passed outright; 14 passed after in-boundary additive repairs recorded below; 4 deferred with named blockers in the deferral table. Converted 170+ `## Related Plans` bullets across nine specs and eight implementation docs plus 12 repository-map rows to the retired citation form; live-path prose pointers in implementation-doc purpose sections and skill provenance lines were left verbatim and must be converted before physical deletion. Physical deletion is a dedicated follow-up; the command-mirror plan's deletion additionally waits for the active 2026-08-24 command-runtime plan, which cites it as a source. |
| 2026-09-24 | Maintenance (harvest repairs in the same wave) | tree at `c894059` | Seven harvested lessons added to `docs/lessons.md` (delegated-slice RED, upstream-primitive probing, canonicalizer-is-not-validator, UTF-8 pipe decoding, short-transcript tail fixture, gate-child release handshake, transplant end-state); four implementation-doc sentences (PeekNamedPipe rejection in 05; debug-capture downgrade order, rejected start-token alternatives, serial direct coverage in 04); repository-map rows for `docs/coalescing.md`, three runbooks, six skills, and `bin/check-dom15-fixtures`; Diataxis gap flags in `docs/README.md`; the registry's equivalence-ledger pointer pinned to source `6d19465`. Each repair is reversible and evidence-backed by the plan it harvests. |
| 2026-09-24 | Process (rule promotion, owner-ratified) | tree at `c894059` | Promoted the all-eligible plan-tier sweep rule into the Fold Unit bullet above and `skills/coalescing/SKILL.md` §4 after diagnosing why the plan tier never converged: four sweeps each retired exactly four oldest candidates (an unwritten norm inherited from the first batch) against inflow near nine plans a week, while SimpleBroker's sweeps harvest every eligible candidate with parallel audits. |
| 2026-09-23 | Plans (soft retirement) | `33e13ee`, `3cae1f4` | Soft-retired the next four oldest eligible non-exemplar plans: stale asset cleanup, the client-module split, schema-shim retirement, and the state/SQL dialect seam. Asset cleanup and the shim deletion had no durable remainder. The client split and dialect seam were absorbed into [TAUT-8.3], [TAUT-12.2], [TAUT-12.1], [TAUT-10], and the architecture note. Repaired two in-boundary rationale gaps from current code: public `_models.py` dataclasses set `__module__` to `taut.client`, and `dialect_for_taut_target()` accepts only a Taut-resolved target. Deviation logs were absent because these plans predate that section, and no pending spec proposal remains. Lessons were not owed. Spec, architecture, and repository-map backlinks now use retained cues. Sources are byte-identical to HEAD and reachable from `origin/main`. No plan was deleted. Remaining eligible: 99. Apparatus-share cue: 0 of 9 active plans is process-subject. |
| 2026-09-23 | Plans (physical deletion) | `348eae9`, `24dc2bc`, `3cae1f4` | Dedicated second-step deletion of the four plans soft-retired 2026-08-24: implementation-review follow-ups, the Postgres extension, member identity/addressing, and indexed latest-timestamp lookup. Re-checked from the current tree: those plans have no pending spec-proposal section; maintained specs, the architecture note, and the repository map already use retired citations; each source is reachable from `origin/main` and was byte-identical to the deleted file. Removed the four status-index rows and plan files and retained the Retired Plans ledger rows. Historical plan-to-plan paths stay in immutable plan sources. No `retired-pending` plans remain. Eligible completed/superseded non-exemplars: 103. Apparatus-share cue: 0 of 9 active plans is process-subject. The plan-index gate passed on the resulting index. |
| 2026-09-23 | Promotion | `70ac379e2` | No skill or durable guidance promoted. Re-derived the cold window 2026-07-27..2026-08-18: xdist/scheduler containment, Summon readiness and input handoff, and causal time-oracles each gained August citations and remain owner-gated. Dependency-floor wording stays with the active compatibility work. No promotion watermark advanced. |
| 2026-09-23 | Lessons | `70ac379e2` | Examined all 34 cold entries dated 2026-07-27..2026-08-18. The declared colon derivation saw 33; `2026-08-08 (revised 2026-08-13)` hid one entry, repaired to `2026-08-08:` so the derivation counts it. Folded 2 already-owned entries after text fidelity, symbol liveness, and code parity: stale mention reply actions into [IAN-7.4]/[IAN-9] (`peek_one` miss omits the action; `show_message()` is not the check), and continuity-token classification into [MCP-10]. Kept 32 verbatim as named candidates: Summon/TUI lifecycle and input ownership; Windows process and pipe identity; time budgets versus behavior oracles; release, publication, and CI evidence; dependency-floor semantics; extension package ownership; and the narrow `uv --project` and `__all__` tooling facts. No golden rule, principle, runbook, or skill was edited. Watermark 2026-07-14 → 2026-08-18. Published source `70ac379e2` contains both removed raw entries. Fourteen entries dated 2026-08-25..2026-09-22 stay ahead of the watermark inside the age floor. |
| 2026-08-24 | Maintenance (deferred process alignment) | — | The skill-prescribed direct `bin/coalesce-check` invocation fails under this machine's ambient `python3` because its `dataclass` lacks `slots`; `uv run bin/coalesce-check` passes with all cues resolved. Changing the prescribed verification invocation is a future-process change outside this Class 2 sweep. Owner: human owner. Reconsider in an authorized Class 3+P skill/gate alignment unit; current sweeps must use the project environment explicitly. |
| 2026-08-24 | Plans (soft retirement) | `348eae9`, `24dc2bc`, `3cae1f4` | Soft-retired the next four oldest eligible non-exemplar plans after two independent current-tree harvest audits: implementation-review follow-ups, the Postgres extension, member identity/addressing, and indexed latest-timestamp lookup. All deviation/spec-proposal debt is closed; durable rationale is absorbed into [TAUT-*], [IAN-*], and the architecture note; applicable lessons are present or explicitly not owed; every spec backlink and maintained repository-map/architecture path now uses a retained cue. Repaired one in-boundary rationale gap by documenting the live O(N) decoded-retention invariant for `log(limit=N)` after verifying its bounded deque implementation. The first-slice GitHub-only/no-root-extra, schema-cutover, old dependency-floor, and plan-era mechanics were judged transitional. Every source is byte-identical and reachable from `origin/main`; no plan was deleted. Remaining eligible: 91. Apparatus-share cue: 0 of 5 live plan rows is process-subject. Project-environment plan-index, path, docs-reference, coalescing, and diff gates rerun green. |
| 2026-08-24 | Lessons | `f17612b` | Checked-deferred: 81 dated entries, 33 past the 2026-07-14 watermark, and 0 cold eligible because the oldest is 2026-07-27 and remains inside the 30-day age floor. All entries remain verbatim; no watermark advanced. Project-environment coalescing and docs-reference gates rerun green. |
| 2026-08-24 | Promotion | `f17612b` | No skill or durable guidance promoted. The three existing candidates remain; the 33 post-watermark entries add hot or product-owned release, TUI, and Summon evidence but no new stable unowned workflow theme at threshold. All durable-guidance amendments remain separately classed and owner-gated; no promotion watermark advanced. |
| 2026-08-14 | Plans (physical deletion) | `f1259c0`, `33e13ee`, `dadd324` | Dedicated second-step deletion of the four plans soft-retired earlier on 2026-08-14: foundation, 0.1.1 hardening, initial GitHub Actions release workflows, and initial release helper. An independent agent re-verified every harvest gate; each retained source is reachable from `origin/main` and contains a byte-identical plan; maintained specs and implementation notes contain only retired citations. Removed the four status-index rows and plan files, retained all four Retired Plans ledger rows, and left historical plan-to-plan paths in immutable excluded plan sources. No `retired-pending` plans remain; 66 completed/superseded non-exemplars remain eligible. Project-environment plan-index, path, docs-reference, coalescing, and diff gates rerun green. |
