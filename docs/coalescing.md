...
| Plans | 2026-08-08, base `9410b6b` | 66 indexed plans: 54 completed, 4 superseded, 4 retired-pending, 2 active, 1 draft, 1 exemplar; 58 completed/superseded non-exemplars remain eligible against threshold 8 | Checked-deferred after the first soft-retirement batch: retirement is batch-limited by per-plan four-gate re-verification; no blocker class was found in the remaining set, it is volume-limited | The next authorized sweep or completion boundary gates the next batch; the physical-deletion follow-up for the 4 retired-pending plans requires a fresh current-tree gate re-check with verified retrieval from each source SHA, per the two-step rule |
